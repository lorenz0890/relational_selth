...
class LinkPrediction(Module):
    ...
    def __init__(self, embedding_size: int):
        super().__init__()
        self.hidden = Linear(2 * embedding_size, HIDDEN, bias=False) #deactivated bias
        self.output = Linear(HIDDEN, 1, bias=False) #deactivated bias
...
class NodePrediction(Module):
    ...
    def __init__(self, embedding_size: int, out_size: int):
        super().__init__()
        self.hidden = Linear(embedding_size, HIDDEN, bias=False) #deactivated bias
        self.output = Linear(HIDDEN, out_size, bias=False) #deactivated bias
...
class T12(Module):
    ...
    def __init__(self, total_nodes: int, total_events: int, previous_embed_size: int):
        ...
        self.bn = BatchNorm1d(self.agg_features, affine=False) #deactivated affine transformation
        self.w1 = Linear(self.agg_features, self.agg_features, bias=False) #deactivated bias
        out_size = self.agg_features + previous_embed_size
        self.w2 = Linear(out_size, out_size, bias=False) #deactivated bias
...
class Model(Module):
    ...
    def __init__(self, flavour: str, total_nodes: int, total_events: int, classes = 0):
        ...
        #torch.ones(total_nodes, EMBED), #remove the uniqueness stemming from the random initialization alone, which aready assigns a unique node emebding. We want to measure only the expressivity stemming from the message passing here.
        persistent=True)
